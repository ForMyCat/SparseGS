...
# OUR
from BMD_utils import ImageandPatchs, ImageDataset, generatemask, getGF_fromintegral, calculateprocessingres, rgb2gray,\
    applyGridpatch
...
def run_mono_model(dataset, option):
    ...
    if option.depthNet == 0:
        ...
        leres_model_path = './BoostingMonocularDepth/pix2pix/checkpoints/mergemodel/res101.pth'
    ...
    for image_ind, images in enumerate(dataset):
        print('processing image', image_ind, ':', images.name, type(images.name))
        ...
        if option.R0 or option.R20:
            path = os.path.join(result_dir, str(images.name))
...
def get_monodepth(data_dir, output_dir):
    parser = argparse.ArgumentParser()

    parser.add_argument('--data_dir', type=str, required=False,default=data_dir,  help='input files directory '
                                                                    'Images can be .png .jpg .tiff')
    parser.add_argument('--output_dir', type=str, required=False,default=output_dir, help='result dir. result depth will be png.'
                                                                      ' vides are JMPG as avi')
    parser.add_argument('--savepatchs', type=int, default=0, required=False,
                        help='Activate to save the patch estimations')
    parser.add_argument('--savewholeest', type=int, default=0, required=False,
    ...
    parser.add_argument('--R0', action='store_true')
    parser.add_argument('--R20', action='store_true')
    parser.add_argument('--Final', default=True)
    parser.add_argument('--max_res', type=float, default=np.inf)
    ...
    # Run pipeline
    run_mono_model(dataset_, option_)